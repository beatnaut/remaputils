...
##' emailParams <- list("sendEmail" = TRUE,"from" = "x","emailList" = "y","isLocal" = FALSE, "subject" = "Example Alert")
...
alertEmail <- function(session,
    ...
    greeting <- "Dear Team"
    message <- "This is a autogenerated alert email triggered. Find below the alert details"

   ## Email Params overrides
   if(!is.null(emailParams[["greeting"]])) {
    greeting <- emailParams[["greeting"]]
   }
   if(!is.null(emailParams[["message"]])) {
