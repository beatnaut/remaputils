...
sepCfrmP <- function(res, ctypeF="OPT", cp=c("C","P"), xtractFirst=c("EQUITY"), xtractLast=c("COMDTY","INDEX"), nameMatch="\\b(CALL|PUT|CALLS|PUTS)\\b", sweep=FALSE) {
  ...
  resources <- resources %>%
    dplyr::rowwise() %>%
    dplyr::mutate(symbolString=as.character(tail(sapply(str_extract_all(symbol,"((\\b|[:alnum:])+[:alpha:]+[:digit:]+(\\b|[:alnum:]))|((\\b|[:alnum:])+[:digit:]+[:alpha:]+(\\b|[:alnum:]))"),function(x) x[stringr::str_detect(x,paste(cp,collapse="|"))]),1))) %>%
    dplyr::mutate(
    callputF=case_when(
        ...
        !stringr::str_detect(toupper(symbol),paste(c(xtractFirst,xtractLast),collapse="|")) ~ stringr::str_sub(str_extract(toupper(name),nameMatch),1,1)    
    )
    ) %>%
  dplyr::mutate(callputF=if_else((is.na(callputF)|!callputF %in% cp) & stringr::str_detect(toupper(symbol),paste(xtractLast,collapse="|")),stringr::str_sub(symbolString,1,1),callputF)) %>% ##sometimes the index is ordered like equity
  dplyr::mutate(callput=case_when(
     callputF=="C" ~ TRUE,
     callputF=="P" ~ FALSE
     )
  ) %>%
